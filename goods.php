            ...
            <table class="table table-sm mb-3">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        $use_cm_for = [1, 5, 6, 13];
                        $raw = (string)$product['size'];
                        $parts = array_filter(array_map('trim', explode(',', $raw)), function($v){ return $v !== ''; });
                        $first = isset($parts[0]) ? $parts[0] : '';
                        // 単位付与
                        if ($first !== '' && !preg_match('/(cm|号)$/u', $first)) {
                            if (preg_match('/^\d+(?:\.\d+)?$/', $first)) {
                                $first .= in_array($bunrui_id, $use_cm_for, true) ? 'cm' : '号';
                            }
                        }
                        echo h($first);
                        ?>
                        <input type="hidden" name="size" value="<?php echo h($first); ?>">
</td>
                </tr>
                <?php endif; ?>
                </tbody>
