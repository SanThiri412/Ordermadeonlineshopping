<?php
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'){
        ...
        if($cardnumber ===''){
            ...
            $errs['cardnumber'] = 'カード番号は14桁～16桁の半角数字で入力してください。';
            $errs['err']="入力ミスがありました。";
        }
...
<body>
    ...
    <div class="container mt-4 p-4 w-50 border rounded">
        <div class="hero-section text-center">
            ...
            <?php if(!empty($errs["err"])) : ?>
                <div>
                    <span style="color:red"><?= @$errs['err'] ?></span>
                </div>
            <?php endif; ?>
            *空欄の場合、既存の情報で保存します。
        </div>
            ...
            <div class="mb-3">
                <label for="exampleInputtext" class="form-label">郵便番号：　*ハイフン(-)を入れて入力してください。</label>
                <div class="row">
                    <div class="col-5"><input type="text" name="postcode" pattern="\d{3}-\d{4}" class="form-control" value=<?= $member->postcode ?> placeholder="xxx-xxxx"></div>
                </div>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label for="exampleInputtext" class="form-label">電話番号：　*ハイフン(-)を入れて入力してください。</label>
                <div class="row">
                    <div><input type="text" name="tel" class="form-control" pattern="\d{2,4}-\d{4}-\d{4}" value=<?= $member->tel ?> placeholder="xxx-xxxx-xxxx">
                </div>
            </div>
            <div class="mb-3">
