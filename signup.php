<?php
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'){
        ...
        if(empty($kanaName)){
            $errs['kanaName']='フリガナを入力してください。';
        }
        if(empty($nickname)){
            $errs['nickname']='ニックネームを入力してください。';
        }
        if(!preg_match('/\A\d{3}-\d{4}\z/', $postcode)){
            $errs['postcode']='郵便番号は「123-4567」のように入力してください。';
        }
        if(empty($address)){
        ...
        if(empty($tel1)||empty($tel2)||empty($tel3)){
            $errs['tel']='電話番号を入力してください。';
        }else if
        (!preg_match('/\A\d{2,4}\z/', $tel1)||
        !preg_match('/\A\d{1,4}\z/', $tel2)||
        !preg_match('/\A\d{3,4}\z/', $tel3)){
            $errs['tel']='電話番号は半角数字2～4桁、1～4桁、4桁で入力してください。';
        }
         if(empty($cardnumber)){
            $errs['cardnumber']='カード番号を入力してください。';
        }else if (!preg_match('/\A\d{14,16}\z/', $cardnumber)) {
            $errs['cardnumber'] = 'カード番号はハイフンなしの14桁～16桁の半角数字で入力してください。';
        }
        //ここ 
    ...
    <div class="container mt-4 p-4 w-50 border rounded">
        <form action="signup.php" method="POST" enctype="multipart/form-data" class="form-horizontal">
            ...
            <h2 class="text-center md-5">新規会員登録</h2>
                <div class="mb-3">
                    メールアドレス*
                    <input type="email" class="form-control" name="email" value="<?= @$email ?>" placeholder="例)test01@jec.ac.jp" >
                    <span style="color:red"><?= @$errs['email'] ?></span>
                </div>
                <div class="mb-3">
                    お名前*
                    <input type="text" class="form-control" name="name" value="<?= @$name ?>" placeholder="例)電子太郎" >
                    <span style="color:red"><?= @$errs['name'] ?></span>
                </div>
                <div class="mb-3">
                    フリガナ*
                    <input type="text" class="form-control" name="kanaName" value="<?= @$kanaName ?>" placeholder="例)でんしたろう" >
                ...
                <div class="mb-3">
                    パスワード*
                    <input type="password" class="form-control" minlength="4" name="password"  >
                ...
                <div class="mb-3">
                    電話番号*
                    <div class="row">
                    <div class="col-2"><input type="text" class="form-control" name="tel1" placeholder="xxx" value="<?= @$tel1 ?>" ></div>-
                    <div class="col-2"><input type="text" class="form-control" name="tel2" placeholder="xxx" value="<?= @$tel2 ?>" ></div>-
                    <div class="col-2"><input type="text" class="form-control" name="tel3" placeholder="xxx" value="<?= @$tel3 ?>" ></div>
                    <span style="color:red"><?= @$errs['tel'] ?></span>
                    </div>
                </div>
                <div class="mb-3">
                    郵便番号*
                    <input type="text"  class="form-control" name="postcode" value="<?= @$postcode ?>"placeholder="例)123-4567" >
                    <span style="color:red"><?= @$errs['postcode'] ?></span>
                </div>             
                <div class="mb-3">
                    住所*
                ...
                <div class="mb-3">
                    カード番号*
                    <input type="text" class="form-control" title="カード番号を入力" name="cardnumber" value="<?= @$cardnumber ?>"  >
                    <span style="color:red"><?= @$errs['cardnumber'] ?></span>
                </div>
                <div class="mb-3">
                    <label for="exampleFormControlTextarea1" class="form-label">自己紹介</label>
                    <textarea class="form-control" name="self_introduction" rows="3"><?= @$self_introduction ?></textarea>     
                </div>
