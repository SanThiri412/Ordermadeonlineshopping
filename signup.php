    ...
    <div class="container mt-4 p-4 w-50 border rounded">
         <form action="login.php" method="POST">
            <h2 class="text-center md-5">新規会員登録</h2>
                <div class="mb-3">
                    メールアドレス*
                    <input type="email" class="form-control" title="メールアドレスに「＠」を挿入してください"name="email" value="<?= @$email ?>" placeholder="例)test01@jec.ac.jp" >
                    <span style="color:red"><?= @$errs['email'] ?></span>
                </div>
                <div class="mb-3">
                    お名前*
                    <input type="text" class="form-control" name="name" value="<?= @$membername ?>"placeholder="例)電子太郎">
                    <span style="color:red"><?= @$errs['name'] ?></span>
                </div>
                <div class="mb-3">
                    フリガナ*
                    <input type="text"  class="form-control" name="kananame" value="<?= @$membername ?>"placeholder="例)でんしたろう">
                    <span style="color:red"><?= @$errs['kananame'] ?></span>
                </div>
                <div class="mb-3">
                    ニックネーム*
                    <input type="text"  class="form-control" name="nickname" value="<?= @$membername ?>"placeholder="例)電子太郎" >
                    <span style="color:red"><?= @$errs['nickname'] ?></span>
                </div>
                <div class="mb-3">
                    パスワード*
                    <input type="password"  class="form-control" minlength="4" name="password" value="<?= @$password ?>" >
